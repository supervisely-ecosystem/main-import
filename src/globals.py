...
import_as_links = (
    os.environ.get("IMPORT_AS_LINKS")
    or os.environ.get("modal.state.importAsLinks", "false")
).lower() == "true"
default_ds_name = f"dataset {datetime.datetime.now().strftime('%Y-%m-%d %H-%M-%S')}"
dataset_name = os.environ.get("DATASET_NAME") or os.environ.get(
    "modal.state.datasetName", default_ds_name
)
if dataset_name == "":
...
if input_paths is None:
    input_paths = os.environ.get("INPUT_PATH")
